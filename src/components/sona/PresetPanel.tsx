...
const getBehaviorText = (preset: Preset) => {
  const behavior = (preset as any).behavior;

  if (!behavior) return null;

  const glow = Math.round((behavior.glowSize ?? 0.75) * 100);
  const trail = (behavior.trailDuration ?? 3).toFixed(1);

  return `Glow ${glow}% • Trail ${trail}s`;
};
...
export const PresetPanel: React.FC<PresetPanelProps> = ({
  ...
  const handleSavePreset = useCallback(() => {
    const newPreset = presetManager.createPreset({
      ...
      // preparado para a próxima etapa do sistema de presets
      behavior: {
        glowSize: 0.75,
        trailDuration: 3,
        motionResponse: 0.7,
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="grid grid-cols-2 gap-2">
        {presets.map((preset) => {
          const behaviorText = getBehaviorText(preset);
